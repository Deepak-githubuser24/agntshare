...
function loadConfig() {
    ...
    if (!apiKey) {
        throw new Error("AGENTSHARE_API_KEY environment variable is required.\n" +
...
async function apiRequest(config, method, path, body) {
    ...
    if (config.agentRole)
        headers["x-agent-role"] = config.agentRole;
...
function selectFromJSON(data, keys, path) {
...
export function createAgentShareServer(config) {
        ...
        tools: [
            {
                ...
                description: "Upload content or structured agent memory/state to AgentShare and mint a short pathway token. " +
            ...
            },
            {
                ...
                description: "Resolve a pathway token to retrieve shared content or structured agent state. " +
            ...
            },
            {
                ...
                description: "Revoke a pathway token to permanently destroy access. " +
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        if (name === "agentshare_share") {
            ...
            const upload = await apiRequest(config, "POST", "/upload", {
                filename,
                contentType,
            ...
            const mint = await apiRequest(config, "POST", "/token", {
        ...
        if (name === "agentshare_resolve") {
            ...
            if (intent)
                queryParams.set("intent", intent);
            if (keys && keys.length > 0)
                queryParams.set("keys", keys.join(","));
            if (path)
                queryParams.set("path", path);
            const queryString = queryParams.toString() ? `?${queryParams.toString()}` : "";
            const resolved = await apiRequest(config, "GET", `/resolve/${token}${queryString}`);
        ...
        if (name === "agentshare_revoke") {
            ...
            const result = await apiRequest(config, "DELETE", `/token/${token}`);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
        ...
        if (uri.startsWith("agentshare://token/")) {
            ...
            const resolved = await apiRequest(config, "GET", `/resolve/${token}`);
            const fileRes = await fetch(resolved.streamUrl);
            if (!fileRes.ok) {
